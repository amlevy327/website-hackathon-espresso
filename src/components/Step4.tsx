...
const Step4 = ({ mode, isCompleted, onComplete }: StepProps) => {
  ...
  const handleBridgeEth = async () => {
    ...
    if (!arbSepoliaBalance || Number(arbSepoliaBalance.value) <= parseEther('0.002')) {
      alert('Insufficient ETH on Arbitrum Sepolia for gas and transaction. You need at least 0.002 ETH.');
      return;
    }
    ...
    writeContract({
      ...
      value: parseEther('0.002'), // 0.002 ETH
      chainId: 421614, // Arbitrum Sepolia
    });
  };
